...
The things that are stuck in your chest, time won't solve them. It just pastes the label of oblivion.`)
    ...
    useEffect(() => {
        tanzi.id++;
        tanziFunction(delay, str, fontSize);
    }, [str, fontSize, delay]);
    return <div style={{display: "flex", flexFlow: 'column', alignItems: 'center'}}>
        <div>
            Font Size:<input type={"range"} onChange={e => setFontSize(Number(e.target.value))} value={fontSize}
                             min={36} max={128}/>{fontSize}{`\u00a0\u00a0\u00a0`}
            Text Display Delay:<input type={"range"} onChange={e => setDelay(Number(e.target.value))} value={delay}
                                      min={10} max={1000}/>{delay}
        </div>
